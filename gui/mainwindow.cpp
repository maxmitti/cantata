...
MainWindow::MainWindow(QWidget* parent)
	: QMainWindow(parent), prevPage(-1), lastState(MPDState_Inactive), lastSongId(-1), autoScrollPlayQueue(true), singlePane(false), shown(false), currentPage(nullptr)
#ifdef QT_QTDBUS_FOUND
	  ,
	  mpris(nullptr)
#endif
	  ,
	  statusTimer(nullptr), playQueueSearchTimer(nullptr)
#if !defined Q_OS_WIN && !defined Q_OS_MAC
	  ,
	  mpdAccessibilityTimer(nullptr), showMenubarAction(nullptr)
#endif
	  ,
	  contextTimer(nullptr), contextSwitchTime(0), connectedState(CS_Init), stopAfterCurrent(false), responsiveSidebar(false)
#if defined(Q_OS_WIN) && QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
	  ,
	  thumbnailTooolbar(0)
#endif
{
	...
	searchPlayQueueAction->setShortcut(QKeyCombination(Qt::ControlModifier | Qt::ShiftModifier, Qt::Key_F));
#ifdef ENABLE_HTTP_STREAM_PLAYBACK
	...
	expandAllAction->setShortcut(QKeyCombination(Qt::ControlModifier, Qt::Key_Down));
...
#define TAB_ACTION(A) A->icon(), A->text(), A->text()
	Qt::Key sidebarPageShortcutKey = Qt::Key_1;
...
MainWindow::~MainWindow()
{
	...
	if (Utils::useSystemTray()) {
		...
		Settings::self()->saveStartHidden(trayItem->isActive() && Settings::self()->minimiseOnClose() && ((isHidden() && Settings::SS_ShowMainWindow != startupState) || (Settings::SS_HideMainWindow == startupState)));
	}
	Settings::self()->save();
...
void MainWindow::showAboutDialog()
{
	QMessageBox::about(this, tr("About Cantata"),
	                   tr("<b>Cantata %1</b><br/><br/>MPD client.<br/><br/>"
	                      "&copy; 2011-2022 Craig Drummond, 2024 Cantata Contributors<br/>Released under the <a href=\"http://www.gnu.org/licenses/gpl.html\">GPLv3</a>")
	                                   .arg(PACKAGE_VERSION_STRING)
	                           + QLatin1String("<br/><br/>") + tr("Please refer to <a href=\"https://github.com/nullobsi/cantata/issues\">Cantata's issue tracker</a> for a list of known issues, and to report new issues.") + QLatin1String("<br/><br/><i><small>") + tr("Based upon <a href=\"http://lowblog.nl\">QtMPC</a> - &copy; 2007-2010 The QtMPC Authors<br/>") + tr("Context view backdrops courtesy of <a href=\"http://www.fanart.tv\">FanArt.tv</a>") + QLatin1String("<br/>") + tr("Context view metadata courtesy of <a href=\"http://www.wikipedia.org\">Wikipedia</a> and <a href=\"http://www.last.fm\">Last.fm</a>") + QLatin1String("<br/><br/>") + tr("Please consider uploading your own music fan-art to <a href=\"http://www.fanart.tv\">FanArt.tv</a>") + QLatin1String("</small></i>"));
}

bool MainWindow::canClose()
{
	if (onlinePage->isDownloading() && MessageBox::No == MessageBox::warningYesNo(this, tr("A Podcast is currently being downloaded\n\nQuitting now will abort the download."), QString(), GuiItem(tr("Abort download and quit")), GuiItem("Do not quit just yet"))) {
		return false;
	}
...
void MainWindow::outputsUpdated(const QList<Output>& outputs)
{
	...
	if (menuItems != inCurrentPartitionMpd || menuMoveableItems != inOtherPartitionMpd) {
		...
		for (const Output& o : out) {
			...
			act->setShortcut(QKeyCombination(Qt::ControlModifier | Qt::AltModifier, nextKey(i)));
		}
		menu->addSeparator();
	...
	if (connections.count() < 2) {
		...
		if (menuItems != cfg) {
			...
			for (const MPDConnectionDetails& d : connections) {
				...
				act->setShortcut(QKeyCombination(Qt::ControlModifier, nextKey(i)));
			}
		}
	}
...
void MainWindow::updateSettings()
{
	...
	if (wasGrouped != playQueue->isGrouped() || (playQueue->isGrouped() && (wasAutoExpand != playQueue->isAutoExpand() || wasStartClosed != playQueue->isStartClosed()))) {
...
void MainWindow::deleteAPartition()
{
	...
	if (act) {
		...
		if (MessageBox::Yes == MessageBox::warningYesNo(this, tr("Are you sure you wish to remove partition \"%1\"?\n\nThis cannot be undone.").arg(name), tr("Remove Partition"), StdGuiItem::remove(), StdGuiItem::cancel())) {
...
#endif
	                                QLatin1String("<p><table>") + tr("<tr><td colspan=\"2\"><b>Server</b></td></tr>"
	                                                                 ...
	                                                                 "<tr><td align=\"right\">Tags:&nbsp;</td><td>%7</td></tr>")
	                                                                      ...
	                                                                      .arg(handlers.join(", "))
	                                                                      .arg(tags.join(", "))
	                                + QLatin1String("<tr/>") + tr("<tr><td colspan=\"2\"><b>Database</b></td></tr>"
	                                                              ...
	                                                              "<tr><td align=\"right\">Updated:&nbsp;</td><td>%5</td></tr>")
	                                                                   ...
	                                                                   .arg(Utils::formatDuration(MPDStats::self()->dbPlaytime()))
	                                                                   .arg(QLocale().toString(dbUpdate, QLocale::ShortFormat))
...
void MainWindow::updateStatus(MPDStatus* const status)
{
	...
	if (MPDState_Stopped == status->state() || MPDState_Inactive == status->state()) {
		...
		nowPlaying->setRange(0, 0 == status->timeTotal() && 0 != current.time && (current.isCdda() || current.isCantataStream()) ? current.time : status->timeTotal());
	...
	if (MPDState_Inactive != status->state() && (MPDState_Inactive == lastState || (MPDState_Stopped == lastState && MPDState_Playing == status->state()) || lastSongId != status->songId())) {
		emit currentSong();
	}
...
{
	if (!Settings::self()->playQueueConfirmClear() || MessageBox::Yes == MessageBox::questionYesNo(this, tr("Remove all songs from play queue?"))) {
...
void MainWindow::toggleContext()
{
	if (songInfoButton->isVisible()) {
		if ((MPDState_Playing == MPDStatus::self()->state() && !songInfoAction->isChecked()) || (MPDState_Stopped == MPDStatus::self()->state() && songInfoAction->isChecked())) {

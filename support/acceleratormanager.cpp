...
void AcceleratorManagerPrivate::manageWidget(QWidget* w, Item* item)
{
	...
	if (qobject_cast<QComboBox*>(w) || qobject_cast<QLineEdit*>(w) || w->inherits("Q3TextEdit") || qobject_cast<QTextEdit*>(w) || qobject_cast<QAbstractSpinBox*>(w) || w->inherits("KMultiTabBar"))
		return;
	...
	if (label) {
		if (!label->buddy())
			...
			if (label->textFormat() == Qt::RichText || (label->textFormat() == Qt::AutoText && Qt::mightBeRichText(label->text())))
				return;
		}
	}

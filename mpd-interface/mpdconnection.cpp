...
#define DBUG \
	if (debugEnabled) qWarning() << "MPDConnection" << QThread::currentThreadId()
...
MPDConnectionDetails::MPDConnectionDetails()
	: port(6600), dirReadable(false), applyReplayGain(true), allowLocalStreaming(true), autoUpdate(false)
{
}
...
MPDConnection::MPDConnection()
	: isInitialConnect(true), thread(nullptr), ver(0), canUseStickers(false), sock(this), idleSocket(this), lastStatusPlayQueueVersion(0), lastUpdatePlayQueueVersion(0), state(State_Blank), isListingMusic(false), reconnectTimer(nullptr), reconnectStart(0), stopAfterCurrent(false), currentSongId(-1), songPos(0), unmuteVol(-1), isUpdatingDb(false), volumeFade(nullptr), fadeDuration(0), restoreVolume(-1)
{
...
{
	return details.isLocal() || (ver >= CANTATA_MAKE_VERSION(0, 19, 0) && /*handlers.contains(QLatin1String("file")) &&*/
...
{
	QByteArray encodedName = encodeName(constStreamsPlayListName);
	if (sendCommand("playlistdelete " + encodedName + " " + quote(position)).ok && sendCommand("playlistadd " + encodedName + " " + encodeName(MPDParseUtils::addStreamName(url, name))).ok) {
...
void MPDConnection::getStickerSupport()
{
	...
	canUseStickers = response.ok && Utils::listToSet(MPDParseUtils::parseList(response.data, QByteArray("command: "))).contains("sticker");
}
...
MpdSocket::MpdSocket(QObject* parent)
	: QObject(parent), tcp(nullptr), local(nullptr)
{
}
